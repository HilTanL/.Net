    ...
    public class ExcelHelper
    {
        ...
        public void DataTableToExcel(DataTable dt, int rows, int top, int left)
        {
            ...
            for (int i = 1; i <= sheetCount; i++)
            {
                ...
                #region 利用Window粘贴板批量拷贝数据（在Web下面行不通）
        ...
        {
            if (index < 0) return "";
            else if (index > GetSheetCount() - 1) return "";

            if (workBook == null) return "";
            else
            {
                Excel.Worksheet sheet = workBook.Sheets.get_Item(index) as Excel.Worksheet;

                if (sheet != null) return sheet._CodeName;
                else return "";
            }
        }
        ...
        public void OutputExcelFile()
        {
            ...
            try
            {
                workBook.SaveAs(outputFile, missing, missing, missing, missing, missing, 
        ...
        public void OutputFile(string format)
        {
            ...
            try
            {
                switch (format)
                {
                    case "HTML":
                        {
                            workBook.SaveAs(outputFile, Excel.XlFileFormat.xlHtml, missing, missing, 
                                missing, missing, Excel.XlSaveAsAccessMode.xlExclusive, missing, missing, 
                                missing, missing,missing);
                            break;
                        }
                    case "CSV":
                        {
                            workBook.SaveAs(outputFile, Excel.XlFileFormat.xlCSV, missing, missing,
                                missing, missing, Excel.XlSaveAsAccessMode.xlExclusive, missing, missing,
                                missing, missing,missing);
                            break;
                        }
                    ...
                    default:
                        {
                            workBook.SaveAs(outputFile, missing, missing, missing, missing, missing, 
                                Excel.XlSaveAsAccessMode.xlExclusive, missing, missing, missing, 
                                missing,missing);
                            break;
                        }
                }
        ...
        public void SaveAsFile(string fileName, string format)
        {
            try
            {
                switch (format)
                {
                    case "HTML":
                        {
                            workBook.SaveAs(fileName, Excel.XlFileFormat.xlHtml, missing, missing, missing,
                                missing, Excel.XlSaveAsAccessMode.xlExclusive, missing, missing, missing,
                                missing,missing);
                            break;
                        }
                    case "CSV":
                        {
                            workBook.SaveAs(fileName, Excel.XlFileFormat.xlCSV, missing, missing, missing, 
                                missing, Excel.XlSaveAsAccessMode.xlExclusive, missing, missing, missing,
                                missing,missing);
                            break;
                        }
